        ...
        <form (ngSubmit)="onSubmit()" #resetForm="ngForm">
          ...
          <div class="mb-2">
            <div class="d-flex justify-content-between align-items-center mb-1">
              <label for="token" class="login-label mb-0">Verification Code</label>
              <button
                type="button"
                class="btn btn-link p-0 lime-link small text-decoration-none"
            ...
            <input
              ...
              [(ngModel)]="resetFormModel.token"
              required
              style="letter-spacing: 2px; text-align: center; font-weight: bold; font-size: 1.1rem !important;"
            />
          </div>

          <div class="mb-3">
            <label for="newPassword" class="login-label">New Password</label>
            <input
              type="password"
              id="newPassword"
              name="newPassword"
              ...
              [(ngModel)]="resetFormModel.newPassword"
              required
              minlength="6"
            />
          </div>
          ...
          <div class="text-center mt-3">
            <a routerLink="/login" class="lime-link small"> Back to Login </a>
          </div>
        </form>
      </div>
    </div>
...
export class ResetPasswordComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.route.queryParams.subscribe((params) => {
      if (params['email']) {
        this.resetFormModel.email = params['email'];
      }
    });
  }
  ...
  resendCode() {
    if (!this.resetFormModel.email) {
      this.toastr.error('Email address is missing.');
      return;
    }
    if (this.resendCooldown > 0 || this.isResending) return;

    this.isResending = true;
    this.authService.forgotPassword(this.resetFormModel.email).subscribe({
      next: () => {
        this.toastr.success('Verification code resent successfully!');
  ...
  onSubmit() {
    if (
      !this.resetFormModel.email ||
      !this.resetFormModel.token ||
      !this.resetFormModel.newPassword
    )
      return;

    this.isLoading = true;
    this.authService.resetPassword(this.resetFormModel).subscribe({
      next: () => {
      ...
      error: (err) => {
        this.toastr.error(err.error?.message || 'Invalid or expired code.');
        this.isLoading = false;
      },
    });
  }
